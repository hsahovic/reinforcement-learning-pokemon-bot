...
from base_code import senders
from base_code.battlelog_parsing import battlelog_parsing
from base_code.login import log_in
from base_code.battle import Battle
...
async def battle_tag(websocket, message, usage, username):
    """
    ...
    for line in lines[1:]:
        try:
            ...
            elif current[1] == "player" and len(current) > 3 and current[3].lower() == username.lower():
...
async def stringing(websocket, message, username, password, usage=0,*,to_challenge=None):
    """
    First filtering function on received messages.
    Handle challenge and research actions.
    :param websocket: Websocket stream.
    :param message: Message received from server. Format : room|message1|message2.
    :param usage: 0: Only recieving. 1 Challenging to_challenge. 2 searching random battles.
    """
    ...
    if string_tab[1] == "challstr":
        ...
        try:
            ...
            pass
    elif string_tab[1] == "pm" and username not in string_tab[2]:
    ...
    if "battle" in string_tab[0]:
        ...
        await battle_tag(websocket, message, usage, username)
